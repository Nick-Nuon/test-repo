                               ...
                               const unsigned char *f, int n)
{
    ...
    // TODO: find out where base64 /w URLs are processed. I assume it happens somewhere but there's no mention of it here. 
    // TODO: Make new tables for SRP data here. 
    ...
    // TODO:  simplify these two checks, we use our own tables. 
    ...
    // TODO?: throughout OpenSSL's codebase , variable names are extremely terse, 
    // I find it less confusing from a comfort POV to use something closer to what we'd do.
    // but it would be probably be a good idea to change back to what they're using eventually
  char unsigned *src = *f, *dst = *t;
  int length = n ;

  const unsigned char *srcend = src + length;
  const unsigned char *srcinit = src;
  const unsigned char *dstinit = dst;
  ...
  while (1) {
    ...
    // There is no mention of endianness in their source code, I assume it is dealt with somewhere else
    ...
    if (idx != 4) { // TODO: figure it if we keep this section, in the original code, everything is 
        // 
      if (idx == 2) {
        uint32_t triple =
